...
const Index = () => {
  ...
  return <>
      ...
      <section id="our-story" className="min-h-screen flex items-center justify-center overflow-hidden relative py-20 sm:py-32">
        <div ref={storyAnimation.ref} className={`relative z-10 backdrop-blur-lg bg-white/10 border border-white/20 rounded-3xl shadow-2xl p-8 sm:p-10 md:p-14 lg:p-16 mx-6 sm:mx-8 max-w-5xl transition-all duration-1000 ${storyAnimation.isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
      ...
      <section id="services" className="relative overflow-hidden py-24 sm:py-32 md:py-40">
        <div ref={servicesAnimation.ref} className={`relative z-10 w-full max-w-3xl mx-auto px-6 sm:px-8 space-y-10 sm:space-y-12 md:space-y-16 transition-all duration-1000 delay-200 ${servicesAnimation.isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          {services.map((service, index) => <ServiceCard key={service.id} title={service.title} content={service.content} index={index} />)}
        </div>
      </section>
      ...
      <section id="contact" className="min-h-screen flex items-center justify-center overflow-hidden relative py-24 sm:py-32">
        <div ref={contactAnimation.ref} className={`relative z-10 w-full max-w-3xl mx-6 sm:mx-8 transition-all duration-1000 delay-300 ${contactAnimation.isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
            ...
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 sm:space-y-7 text-left">
                ...
                field
                      ...
                      <FormControl>
                        <Input placeholder="Your name" {...field} className="bg-white/10 border-white/20 text-white placeholder:text-white/50 min-h-[52px] text-base sm:text-lg" />
                      </FormControl>
                ...
                field
                      ...
                      <FormControl>
                        <Input type="email" placeholder="your@email.com" {...field} className="bg-white/10 border-white/20 text-white placeholder:text-white/50 min-h-[52px] text-base sm:text-lg" />
                      </FormControl>
                      <FormMessage className="text-red-300 text-sm" />
                    </FormItem>} />
                ...
                field
                      ...
                      <FormControl>
                        <Input placeholder="Your company" {...field} className="bg-white/10 border-white/20 text-white placeholder:text-white/50 min-h-[52px] text-base sm:text-lg" />
                      </FormControl>
                      <FormMessage className="text-red-300 text-sm" />
                    </FormItem>} />
                ...
                field
                      ...
                      <FormControl>
                        <Textarea placeholder="Tell us about your data and AI needs..." {...field} className="bg-white/10 border-white/20 text-white placeholder:text-white/50 min-h-[160px] text-base sm:text-lg resize-none" />
                      </FormControl>
                      <FormMessage className="text-red-300 text-sm" />
                    </FormItem>} />
                
                <Button type="submit" className="w-full bg-white/20 hover:bg-white/30 text-white border border-white/30 transition-all duration-300 hover:scale-105 hover:shadow-lg min-h-[58px] text-lg sm:text-xl font-semibold touch-manipulation mt-8">
                  Send Message
                </Button>
              </form>
            </Form>
